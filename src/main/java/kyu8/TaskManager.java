...
class TaskManager {
    ...
    public Supplier<Task> getTaskSupplier() {
        Supplier<Task> taskSupplier = () -> {if (taskQueues.stream().filter(q -> !q.isEmpty())==null) return null;
            Optional<Queue<Task>> o1 = taskQueues.stream().filter(a -> !a.isEmpty()).findFirst();
            if (!o1.isEmpty()) return o1.get().poll();
            return null;
        };
        return taskSupplier; // write your code here
    }
}
