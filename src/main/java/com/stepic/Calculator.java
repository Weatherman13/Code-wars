...
class Calculator<T extends Number> {
    ...
    public <U extends Number> Calculator<U> eval(Function<? super T, ? extends U> mapper) {
        ...
        var resultValue = mapper.apply(this.value);
        if (resultValue == null) return getBrokenCalculator();
        else {
            var result = of(resultValue);
            return result;
        }
    ...
    public Calculator<T> consume(Consumer<T> consumer) {
        if (this.hasError) return getBrokenCalculator();
