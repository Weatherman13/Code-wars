...
public class OnlineJobSystem {
    public static long calculateNumberOfVacancies(ApplicantRequest request, Collection<JobDescription> jobs) {
        return jobs.stream()
                ...
                .filter(job -> job.getTitle().toLowerCase(Locale.ROOT).contains("java developer"))
                .filter(job -> job.getCompany().toLowerCase(Locale.ROOT).contains(request.getCompany().toLowerCase(Locale.ROOT)))
                .count();
    }
